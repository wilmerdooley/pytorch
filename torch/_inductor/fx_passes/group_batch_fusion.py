...
_CAT_TARGETS = [
    t
    for t in (torch.cat, getattr(torch, "concat", None), getattr(torch, "concatenate", None))
    if t is not None
]
...
def match_cat_linear(node: torch.fx.Node):
    ...
    weight = get_arg_value(node, 1, "weight")
    bias = get_arg_value(node, 2, "bias")
    if not isinstance(cat_in, torch.fx.Node) or not CallFunctionVarArgs(_CAT_TARGETS).match(
        cat_in
    ):
        return None

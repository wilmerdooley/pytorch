    ...
    *,
    ...
    if (
        ...
        torch._check(
            ...
            lambda: "block_mask was created for a smaller length than the query.",
        )
        torch._check(
            kv_len <= block_mask_kv_len,
            lambda: "block_mask was created for a smaller length than the key.",
        )
        torch._check(
            q_len >= block_mask_q_len,
            lambda: "block_mask was created for a larger length than the query.",
        )
        torch._check(
            kv_len >= block_mask_kv_len,
            lambda: "block_mask was created for a larger length than the key.",
        )
